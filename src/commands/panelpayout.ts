...
// Interface để lưu trữ thông tin payout
...
const activePayouts = new Map<string, PayoutRequest>();
const userActiveRequests = new Map<string, string>(); // userId -> service
...
module.exports = {
    ...
    async execute(interaction: any) {
        ...
        const filter = (i: any) => i.customId === 'colorModal';
...
• 10 invites <a:Arrow:1430164794538856580> 3k method (old but working)
    ...
    async handleSelectMenu(interaction: any) {
        if (interaction.customId === 'serviceSelect') {
            ...
            const serviceMap: { [key: string]: { name: string; invites: number; requiresTicket: boolean } } = {
            ...
            if (service.requiresTicket) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: 'Status', value: '🟡 Pending Ticket', inline: true }
                    )
                ...
                if (payoutChannel && payoutChannel.isTextBased()) {
                    const payoutEmbed = new EmbedBuilder()
                        ...
                        .addFields(
                            { name: 'Service', value: service.name, inline: true },
                            { name: 'Invites Required', value: `${service.invites}`, inline: true },
                            { name: 'Ticket Required', value: '✅ Yes', inline: true },
                            { name: 'Status', value: '🟡 Waiting for Ticket', inline: true }
                        )
                        .setTimestamp();
    ...
    async handleConfirmButton(interaction: any) {
...
- Using different VPN is even better.
            ...
            const modalRow = new ActionRowBuilder<TextInputBuilder>().addComponents(accountInput);
    ...
    async handleCancelButton(interaction: any) {
                ...
                if (payoutChannel && payoutChannel.isTextBased()) {
                    const payoutEmbed = new EmbedBuilder()
                        ...
                        .addFields(
                            ...
                            { name: 'Status', value: '✅ Ready for Processing', inline: true }
                        )
                        .setTimestamp();
